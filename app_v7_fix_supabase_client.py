# RMS Sync v2.1 - Full Stack with Supabase Client Fix
import os
import sys
import json
import csv
...
# ============================================================
# Définir APP_DIR et UPLOAD_DIR au début du fichier (Fixes NameError)
...
def parse_file():
    """Endpoint de parsing de fichiers Excel - FIXES MISSING SUPABASE CLIENT ARGUMENT."""
    if 'file' not in request.files:
    ...
    # ============================================================
    # FIX: INITIALISER LE CLIENT SUPABASE ICI
    # ============================================================
    try:
        supabase_url = os.getenv('SUPABASE_URL')
        supabase_key = os.getenv('SUPABASE_KEY')
        ...
        logger.info(f"Supabase client initialized for table type: {table_type}")
        
    except Exception as e:
        logger.error(f"Failed to initialize Supabase client: {str(e)}", exc_info=True)
        return jsonify({
            "error": f"Supabase initialization failed: {str(e)}",
    ...
    if file:
        try:
            ...
            # FIX: Passer le supabase_client comme 4ème argument
            processor = ProcessorFactory.get_processor(table_type, file, hotel_id, supabase_client)
...
if __name__ == '__main__':
    ...
    logger.info("Application démarrée (Full Stack with Supabase Client Fix)")
